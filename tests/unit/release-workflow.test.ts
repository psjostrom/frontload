...
describe("npm publish workflow", () => {
  ...
  it("creates release PRs manually without publishing", () => {
    ...
    expect(stepBlock(job, "Create release PR")).toContain(
    ...
    expect(stepBlock(job, "Create release PR")).toContain(
      'node scripts/create-release-pr.mjs --version "$VERSION_INPUT"',
    );
    expect(stepBlock(job, "Create release PR")).toContain(
      'node scripts/create-release-pr.mjs --bump "$BUMP_INPUT"',
    );
