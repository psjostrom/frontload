...
If the MCP server is unavailable, ask the user to run `frontload doctor` and restart Codex. If `frontload --version` is not available in their shell, ask them to run `npx frontload init` again or install the package globally. For a local source checkout, build Frontload with `pnpm build` and reload the plugin.
