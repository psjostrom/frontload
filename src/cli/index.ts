...
program.command("dossier").argument("<task>").option("--repo <repo>", "repository root", ".").option("--format <format>", "markdown").option("--budget <chars>").option("--max-files <n>", "maximum ranked files to include", parsePositiveInteger, 12).action(async (task, opts) => {
  const repoRoot = resolveRepo(opts.repo);
