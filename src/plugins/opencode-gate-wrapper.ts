...
export function opencodeGatePluginWrapper(preferredAdapterUrl: string | null): string {
  return `// Frontload gate plugin for opencode.
...
const preferredAdapterUrl = ${JSON.stringify(preferredAdapterUrl)};
const adapterRelativePath = ${JSON.stringify(opencodeGateAdapterRelativePath)};
...
export function isGeneratedOpencodeGateWrapper(text: string): boolean {
  return text.includes("dist/src/gate/adapters/opencode.js")
    && text.includes("async function loadAdapter()")
    && !text.includes("src/gate/gate.js")
    && !text.includes("npm root -g")
    && !text.includes("function loadConfig");
}
