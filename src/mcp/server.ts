...
export async function startMcp(repoRoot: string): Promise<void> {
  ...
  server.tool("fl_search", "Search indexed files by task terms. Use instead of broad grep; not for exact regex matching.", { query: z.string(), limit: z.number().default(10) }, async ({ query, limit }) => json({ summary: "Search results from index.", results: await searchIndex(repoRoot, query, limit) }));
  server.tool("fl_read_budgeted", "Read a bounded file excerpt. Use for targeted reads; do not use for binary or unrelated files.", { path: z.string(), query: z.string().optional(), budgetChars: z.number().default(4000) }, async (input) => json(readBudgeted(repoRoot, input.path, input.budgetChars, input.query)));
